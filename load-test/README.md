...
`results/<날짜>/`의 저장 결과는 모두 안정화 작업 이전 소스 또는 당시의
별도 패치 후보를 측정한 과거 자료입니다. 일부 후보의 통과·실패를 비교하는 데에는
사용할 수 있지만 현재 코드의 성능을 대신하지 않습니다.

현재 기능과 Redis 원자성은 132개 테스트로 검증했습니다. 안정화한 소스의 예약
경로와 진단 경로는 지속 처리량을 다시 측정하지 않았으므로 현재 코드에는 처리량이나
p95/p99 수치를 제시하지 않습니다. 현재 판정과 과거 측정 자료의 범위는
[검증·측정 상태](results/BEST.md)에 있습니다.

## 보조 검사

`check_latency.js`는 2026-05-29 안정화 전 지연·포화도 비교에 사용했습니다.
`consumer_throughput.sh`는 합성 바이트를 사용해 발행부터 브로커 확인까지 걸린
시간을 측정합니다. 유효한 Avro 이벤트를 처리하지 않으므로 소비 처리량을
